...
-- SQL: RDMS에서 사용되는 언어 (데이터베이스를 구축, 관리, 활용하기 위한 언어)
-- 참고! SQL은 대소문자를 구별하지 않음
-- 명령어를 구분하기 쉽게 하기 위해 대문자를 작성하는 것(필수 X)
...
-- PRIMARY KEY : 기본키 (중복갑 허용X, NULL 허용X) -> 하나의 테이블당 하나만!
...
-- DROP: 테이블 존재 자체를 없앰
DROP TABLE product;
-- TRUNCATE: 테이블 구조만 남겨놓고 모든 명령 삭제
